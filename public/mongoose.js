...
document.querySelectorAll('#user-list tr').forEach((el) => {
    el.addEventListener('click', function () {
        const id = el.querySelector('td').textContent; // 클릭한 행의 첫번째 셀에 있는 텍스트 콘텐츠(사용자 ID)를 가져옵니다.
...
async function getUser() {
    try {
        ...
        users.map(function (usre) {
            ...
            row.appendChild(td);
            td = document.createElement('td');
            td.textContent = user.age; // 사용자 ID를 셀에 추가
            row.appendChild(td); 
            td = document.createElement('td');
...
async function getComment(id) {
    try {
        ...
        const tbody = document.querySelector('#comment-list tbody'); // 댓글 목록이 표시될 테이블 tbody를 선택
        tbody.innerHTML = ''; // 이전에 추가된 내용 삭제
